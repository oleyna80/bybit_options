...
from sqlalchemy import create_engine, Column, String, DateTime, Numeric, Boolean, Integer
...
class Trade(Base):
    ...
    exec_id = Column(String(100), unique=True, index=True, nullable=False)
...
class TradeLogger:
    ...
    def __init__(
        ...
        # Deduplication
        self.logged_execution_ids: Set[str] = set()
    ...
    async def log_trade(self, execution_data: Dict[str, Any]):
        ...
        try:
            ...
            await asyncio.wait_for(
                self.trade_queue.put(execution_data),
                timeout=1.0
            )
            
            # Mark as logged
            self.logged_execution_ids.add(exec_id)
            
            # Cleanup old IDs (prevent memory leak)
            if len(self.logged_execution_ids) > self.config.dedup_cache_size:
                # Remove oldest 1000 IDs
                to_remove = list(self.logged_execution_ids)[:1000]
                self.logged_execution_ids -= set(to_remove)
                ...
                for trade_data in enriched:
                    ...
                    trade = Trade(
                        ...
                        timestamp=datetime.fromtimestamp(
                            ...
                            tz=timezone.utc
                        ) if trade_data.get("execTime") else datetime.now(timezone.utc),
                        ...
                        iv=float(trade_data.get("_iv", 0)) if trade_data.get("_iv") else None,
                        underlying_price=float(trade_data.get("_underlying_price", 0)) if trade_data.get("_underlying_price") else None,
        ...
        for trade in trades:
            try:
                ...
                exec_time = datetime.fromtimestamp(
                    int(trade.get("execTime", 0)) / 1000,
                    tz=timezone.utc
                ) if trade.get("execTime") else datetime.now(timezone.utc)
                ...
                if existing:
                    ...
                    # Case 3: Position reduced (same direction)
                        ...
                        logger.debug(f"Reduced {symbol}: qty {old_net_qty:.4f}→{new_net_qty:.4f}")
                ...
                logger.error(f"Failed to update position entry for {trade.get('symbol')}: {e}")
                    ...
                    if self.stream_manager:
                        ...
                        if perp_ticker:
                            underlying_price = float(
                                perp_ticker.get("lastPrice") or 
                                perp_ticker.get("markPrice") or 
                                0
                            ) or None
...
async def main():
    ...
    db_url = os.getenv("DATABASE_URL", "postgresql+asyncpg://postgres:password@localhost:5432/trades")
    ...
    try:
        ...
        try:
            ...
            logger.error(f"⚠️  Trade logger init failed: {e}. Continuing without logging.")
            trade_logger = None
